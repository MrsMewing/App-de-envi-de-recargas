let recargas = [
    {
        compañia: "tigo",
        opciones: [
            {
                nombre: "todo incluido",
                recargas: [
                    {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "13",
                        descripcion: "1 día de internet de 800MB, Whatsaap, Facebook, minutos (30) sms (10)",
                        ussd: "*777*--PIN--*7*4*2*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "21",
                        descripcion: "3 días, 2.5GB, mins (75) y sms (20) ilimitado a GT y USA, APPS (facebook, insta etc)",
                        ussd: "*777*--PIN--*7*4*7*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "36",
                        descripcion: "7 días, 5GB, mins (150) y sms (20) ilimitado a GT y USA, APPS (todas)",
                        ussd: "*777*--PIN--*7*4*8*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "60",
                        descripcion: "15 días 7GB, mins (250) y sms (40) ilimitado a GT y USA, APPS (todas)",
                        ussd: "*777*--PIN--*7*4*11*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "115",
                        descripcion: "30 días 14GB, mins (500) y sms (80) ilimitado a GT y USA, APPS (todas)",
                        ussd: "*777*--PIN--*7*2*6*--TELEFONO--*1#"
                    },
                                        {
                        compañia: "tigo",
                        tipo: "todo incluido",
                        precio: "150",
                        descripcion: "30 días 17GB, mins (500) y sms (80) ilimitado a GT y USA, APPS (todas)",
                        ussd: "*777*--PIN--*7*4*10*--TELEFONO--*1#"
                    },
                ]
            },
            {
                nombre: "solo internet",
                recargas: [
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "8",
                        descripcion: "1 día de internet de 800MB y whatsApp con facebook",
                        ussd: "*777*--PIN--*7*2*1*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "16",
                        descripcion: "1 día de internet de 3GB, whatsApp con facebook",
                        ussd: "*777*--PIN--*7*2*2*--TELEFONO--*1#"
                    },
                                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "13",
                        descripcion: "2 días de internet de 1.6GB, whatsApp con facebook",
                        ussd: "*777*--PIN--*7*2*7*--TELEFONO--*1#"
                    },
                                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "19",
                        descripcion: "3 días de internet de 2.5GB, APPS (todas las apps), tigo sports y free fire",
                        ussd: "*777*--PIN--*7*2*5*--TELEFONO--*1#"
                    },
                ]
            },
            {
                nombre: "llamadas y sms",
                recargas: [
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "8",
                        descripcion: "1 día 50 mins y 50 sms",
                        ussd: "*777*--PIN--*7*4*1*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "13",
                        descripcion: "2 dias 120 mins y 120 sms",
                        ussd: "*777*--PIN--*7*4*5*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "13",
                        descripcion: "7 dias 75 mins y 75 sms",
                        ussd: "*777*--PIN--*7*1*6*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "36",
                        descripcion: "7 días 500 mins y 500 sms",
                        ussd: "*777*--PIN--*7*4*6*--TELEFONO--*1#"
                    },
                    {
                        compañia: "tigo",
                        tipo: "internet",
                        precio: "115",
                        descripcion: "30 días ilimitado 1,000 mins y 1,000 sms",
                        ussd: "*777*--PIN--*7*4*9*--TELEFONO--*1#"
                    },
                ]
            },
            {
                nombre: "todo incluido con VIX",
                recargas: []
            },
            {
                nombre: "acceso futbol",
                recargas: []
            }
        ]
    },
    {
        compañia: "claro",
        opciones: [
            {
                nombre: "internet",
                recargas: [
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "8",
                        descripcion: "1 día de internet de 500MB",
                        ussd: "*905*2*1*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "13",
                        descripcion: "2 días de internet de 1.2GB y whatsApp y facebook",
                        ussd: "*905*2*2*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "19",
                        descripcion: "3 días de internet de 2.2GB más todas las redes sociales",
                        ussd: "*905*2*3*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "13",
                        descripcion: "1 día de internet de 3GB y whatsApp y facebook",
                        ussd: "*905*2*4*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "36",
                        descripcion: "7 días de internet de 6GB más todas las redes sociales",
                        ussd: "*905*2*5*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "60",
                        descripcion: "15 días de internet de 8GB más todas las redes sociales",
                        ussd: "*905*2*6*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "115",
                        descripcion: "30 días de internet de 15GB más todas las redes sociales",
                        ussd: "*905*2*7*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "internet",
                        precio: "8",
                        descripcion: "noches de internet ilimitado 10pm a 6am",
                        ussd: "*905*2*8*--TELEFONO--*2024*1#"
                    },
                ]
            }, 
            {
                nombre: "minutos y mensajes Nac. / USA", 
                recargas: [
                    {
                        compañia: "claro",
                        tipo: "minutos y mensajes",
                        precio: "8",
                        descripcion: "1 dia de 50 minutos y 50 sms nac. / USA",
                        ussd: "*905*3*1*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "minutos y mensajes",
                        precio: "13",
                        descripcion: "2 dias de 120 minutos y 120 sms nac. / USA",
                        ussd: "*905*3*2*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "minutos y mensajes",
                        precio: "19",
                        descripcion: "3 dias de 180 minutos y 180 sms nac. / USA",
                        ussd: "*905*3*3*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "minutos y mensajes",
                        precio: "26",
                        descripcion: "7 dias de 225 minutos y 225 sms nac. / USA",
                        ussd: "*905*3*4*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "minutos y mensajes",
                        precio: "50",
                        descripcion: "15 dias de 600 minutos y 600 sms nac. / USA",
                        ussd: "*905*3*5*--TELEFONO--*2024*1#"
                    },
                ]
            },
            {
                nombre: "todo incluido", 
                recargas: [
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "13",
                        descripcion: "1 dia Llamadas Nac./USA +  650 MB + whatsApp, Facebook y claro sports",
                        ussd: "*905*4*1*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "19",
                        descripcion: "2 dias Llamadas Nac./USA + 2 GB + todas las APPS y claro sports",
                        ussd: "*905*4*2*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "36",
                        descripcion: "7 dias Llamadas Nac./USA +  5 GB + todas las Apps + claro sports y music",
                        ussd: "*905*4*3*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "60",
                        descripcion: "15 dias Llamadas Nac./USA +  7 GB + todas las Apps + claro sports y music",
                        ussd: "*905*4*4*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "115",
                        descripcion: "30 dias Llamadas Nac./USA +  12 GB + todas las Apps + claro sports y music",
                        ussd: "*905*4*5*--TELEFONO--*2024*1#"
                    },
                    {
                        compañia: "claro",
                        tipo: "todo incluido",
                        precio: "20",
                        descripcion: "3 dias Llamadas Nac./USA +  2.5 GB + todas las Apps + claro sports y music",
                        ussd: "*905*4*6*--TELEFONO--*2024*1#"
                    },
                ]
            },
            {
                nombre: "redes sociales", 
                recargas: []
            }
        ]
    }
]

export function obtener_opciones_compañia(criterio_busqueda){
    return recargas.filter((opcion) => opcion.compañia.toLocaleLowerCase() == criterio_busqueda.toLocaleLowerCase());
}